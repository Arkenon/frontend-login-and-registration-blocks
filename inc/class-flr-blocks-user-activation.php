...
class Flr_Blocks_User_Activation {
	...
	public function activate_user( string $code, int $user_id ): array {
		...
		$activation_code  = get_user_meta( $user_id, 'flr_blocks_user_activation_code', true );
		...
		if ( $activation_code === $code ) {
			...
			if ( $check_activation ) {
				...
				return array(
					'status'  => true,
					'message' => esc_html_x( "This account has already activated.", "user_already_activated", "flr-blocks" )
				);

			}

			$update = update_user_meta( $user_id, 'flr_blocks_user_activation', 'activated' );
			...
			return array(
				'status'  => true,
				'message' => esc_html_x( "Your account has activated successfully. You can sign in.", "your_account_has_activated", "flr-blocks" )
			);
